...
agg_df = df.groupby(['Product', 'year']).agg(complaint_cnt=('Product', 'count'))
# agg_df.columns = agg_df.columns.droplevel(0)

# count number of companies receiving a complaint for that product per year
agg_df2 = df.groupby(['Product', 'year', 'Company']).agg(cmpny_cnt=('Company', 'count'))
...
agg_df2 = agg_df2.groupby(['Product', 'year']).agg(max_cmpny_cnt=('cmpny_cnt', 'max'))
...
agg_df['max_cmpny_complaint_perc'] = round(agg_df.max_cmpny_cnt/agg_df.cmpny_cnt * 100)

# write result to file
agg_df[['complaint_cnt', 'max_cmpny_cnt', 'max_cmpny_complaint_perc']].to_csv(OUTPUTFILE)
